...
\begin{ghatsnote}
Some mission front ends use the IDL common block name \texttt{COMMON DATI}
with different internal layouts.  The safe practice is to use a fresh GHATS/IDL
session when switching between mission families, especially between the
RXTE/event-list, Huiyan/HXMT, LAXPC, and CZTI production paths.  In IDL, even
asking for \keyword{HELP} can compile the routine and therefore check the
mission-specific \texttt{COMMON DATI} layout.
\end{ghatsnote}
...
\routine{ghats_all} can also read an FFT product and compute the PDS panel
from the individual FFT records, but the light-curve panel is not reliable
for FFT input.
