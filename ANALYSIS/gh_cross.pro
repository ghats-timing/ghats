...
;		TLAG     = flag to return time lags
;       INDEX    = range of selected indices
;       TIME     = range of selected times
;       RATE     = range of selected rates
;       SEL      = array with index selection
;       POISSON  = two-value array with frequency range from which to compute the Poisson correction
;			       (which is then applied)
;       POIVALUE = Real part of noise average cross-spectrum (for manual subtraction)
;		CROSS    = optional output cross spectrum (2-column complex array for values and errors)
;
; EXAMPLE:
...
;		T. Belloni   9 Jun 2021  added CROSS keyword for cross-spectrum output (from M. Méndez)
;			
;
;--------------------------------------------------------------------------
...
;   retall
;endif
if(abs(T1-T2) gt 1.0e-9) then begin
...
if(abs(rmjd01-rmjd02) ge 1.0e-8) then begin
	print,rmjd01-rmjd02
...
;output of cross spectrum
	cross     = complexarr(n_elements(ccsumjk_reb),2)*0.0
	...
	cross[*,1]   = complex(repart_err,impart_err)
