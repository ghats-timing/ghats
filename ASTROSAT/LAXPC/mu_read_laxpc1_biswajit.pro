pro mu_read_laxpc1_biswajit,unit,laxpc_layer,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
if(tag ne canale_corrente1) then begin
	...
	fxbreadm,unit,[time_unit,picol],tempi1,energy1   ; energy here is event word
...
il_mio = where(time_wanted gt tempi1)+1
irowl  = max(il_mio)+1
...
;
   ...
   time  = tempi1(irow-1) ; MU6

; Here channel energy must be found

parola   = energy1(irow-1)
ichan    = 0
layer    = 0
