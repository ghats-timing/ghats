pro mu_read_laxpc2_biswajit,unit,laxpc_layer,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
picol       = where(ttype eq 'FinalEvent')+1

nrows     =fxpar(header,'NAXIS*')
nrows     =nrows(1)
;
; MU6 addition: at first call read in full file to memory using fxbreadm
if(tag ne canale_corrente2) then begin
	if(baryflag eq 1) then begin
		time_unit = fxbcolnum(unit,'barytime')
	endif
	fxbreadm,unit,[time_unit,picol],tempi2,energy2   ; energy here is event word
	canale_corrente2 = tag
endif
;
...
parola   = energy2(irow-1)
ichan    = 0
layer    = 0

;FOR ib = 0,9 DO BEGIN
;  ichan = ichan + 2^(mu_bit_extractor(parola,ib+5))  ; channel is in bits 5-14
;ENDFOR
;FOR ib = 1,4 DO BEGIN
;  layer = layer + 2^(mu_bit_extractor(parola,ib))  ; layers are in bits 1-4
;ENDFOR

; To extract bits 5-14 (channel)
parola2 = parola and 4092       ; zeroes all bits but 5-14
ichan   = ishft(parola2,-2)       ; shifts right by two bits (left is filled with zeros)

; To extract bits 1-4 (layer)
layer   = ishft(parola,-12)     ; shift right by 12 bits (left is filled with zeros)

;print,'***** Channel:',ichan,'    Layer: ',layer   ; test printout
...
;
         ...
         if(tbin gt 0.0d0) then begin
           ...
           if (ibin le (np-1)) then begin
            rdata(ibin) = rdata(ibin) + channels(ichan);*total(layer eq laxpc_layer)   ; * eliminates unwanted layers
            std21       = std21 + channels_std21(ichan);*total(layer eq laxpc_layer)  ; Band 1
            std22       = std22 + channels_std22(ichan);*total(layer eq laxpc_layer)  ; Band 2
            std23       = std23 + channels_std23(ichan);*total(layer eq laxpc_layer)  ; Band 3
           endif
         endif
