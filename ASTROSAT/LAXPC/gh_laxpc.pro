...
;       GH_LAXPC,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_LAXPC,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
common dati,tempi1,energy1,tempi2,energy2,tempi3,energy3,canale_corrente1,canale_corrente2,canale_corrente3,std21,std22,std23 ; common block for keeping the data
...
;	    endif
     	...
     	print,'Usage: gh_laxpc,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_laxpc_biswajit,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information_laxpc_biswajit,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
;
   mu_channel_selection_swift,metafiles,filenames,channels,channels1,channels2, $
...
laxpc_layer=[] ; array with layer coding according to the level2 specification
FOR iu=0,quanti-1 DO BEGIN
	IF(strmid(quali_layers,iu,1) eq '1') THEN laxpc_layer = [laxpc_layer,1,2]
	IF(strmid(quali_layers,iu,1) eq '2') THEN laxpc_layer = [laxpc_layer,3,4]
	IF(strmid(quali_layers,iu,1) eq '3') THEN laxpc_layer = [laxpc_layer,5]
	IF(strmid(quali_layers,iu,1) eq '4') THEN laxpc_layer = [laxpc_layer,6]
	IF(strmid(quali_layers,iu,1) eq '5') THEN laxpc_layer = [laxpc_layer,7]
END
;-------------------------------------------------------------------
;
...
;
   if (i eq 0) then begin
      for j=0,nmetafiles-1 do begin
         sw_first_file = 1
         mu_init_laxpc_biswajit,filenames(i,j),time_offset,sw_first_file, $
                    fields_selected,j,source,observatory, $
                    instrument,mjdrefi,mjdreff,timezero
      endfor
   endif ELSE begin
         sw_first_file = 0
         mu_init_laxpc_biswajit,filenames(i,0),time_offset,sw_first_file, $
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      CASE types(i,j) OF
      'LAXPC1': BEGIN
		 IF(laxpc_unit[0] eq 1) THEN BEGIN
      	 riga    = 0l
     	 mu_read_laxpc1_biswajit,unita,laxpc_layer,tag,rdata,np,cannali, $
	     ...
	     END
       'LAXPC2': BEGIN
		     IF(laxpc_unit[1] eq 1) THEN BEGIN
         	 riga    = 0l
        	 mu_read_laxpc2_biswajit,unita,laxpc_layer,tag,rdata,np,cannali, $
                 		fields_selected,j,tress(i,j),tres_fft,   $
                 		  tstart_fft,tend_fft,$
                  		   riga,channels_std21,channels_std22,channels_std23,nffts
        	 irowl(j) = riga  ; subarrays are passed by value!!
		 ENDIF
   	     END
       'LAXPC3': BEGIN
		    IF(laxpc_unit[2] eq 1) THEN BEGIN
         	 riga    = 0l
        	 mu_read_laxpc3_biswajit,unita,laxpc_layer,tag,rdata,np,cannali, $
                 		fields_selected,j,tress(i,j),tres_fft,   $
                 		  tstart_fft,tend_fft,$
                  		   riga,channels_std21,channels_std22,channels_std23,nffts
        	 irowl(j) = riga  ; subarrays are passed by value!!
...
close,/all
fine_dps:
print,'gh_laxpc: normal termination'
;beep
;
