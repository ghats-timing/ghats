pro gh_laxpc_misra_perunit,infilename,outtype,canalinput,quali_unita,quali_layers,treb,npds,oufilename,ghx=ghx,bands=bands,gti=usergti,sliding=sliding,bary=bary, $
	       wind=wind,wpar=wpar,resp=respfilelist,help=help
;+
; NAME:
;      GH_LAXPC_MISRA_PERUNIT
; PURPOSE:
...
;      GH_LAXPC_MISRA routine unless the per-unit handling in this file is
;      specifically required.
;
; CALLING SEQUENCE:
;       GH_LAXPC_MISRA_PERUNIT
;       GH_LAXPC_MISRA_PERUNIT,parfilename
;       GH_LAXPC_MISRA_PERUNIT,infilename,outtype,channels,units,layers,treb,npds,outfilename[,/muxana][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
;       GH_LAXPC_MISRA_PERUNIT,/HELP
; INPUTS:
...
;       path.  This routine is a specialized per-unit variant kept under
;       ASTROSAT/temp.
...
if(keyword_set(help)) then begin
   print,''
   print,'GH_LAXPC_MISRA_PERUNIT'
   print,''
   print,'Specialized Misra-format AstroSat/LAXPC front-end with per-unit'
   print,'handling. The standard routine is GH_LAXPC_MISRA.'
   print,''
   print,'Usage:'
   print,'  GH_LAXPC_MISRA_PERUNIT'
   print,"  GH_LAXPC_MISRA_PERUNIT, '#parameters.par'"
   print,"  GH_LAXPC_MISRA_PERUNIT, infile, outtype, channels, units, layers, treb, npds, outfile"
   print,''
   print,'Arguments:'
   ...
   IF(n_elements(canalinput) eq 2) THEN BEGIN
	   ...
	   IF(strpos(prova,'.') lt 0) THEN BEGIN  ; integer values == canali
	     ...
	     ENDIF ELSE BEGIN ; real values == energies
			 ...
			 multiple = 0     ; GIGUS
			IF(canalinput[0] ge 0.0) THEN BEGIN   ; here energy is converted from channel using DRM
