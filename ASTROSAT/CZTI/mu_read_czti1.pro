pro mu_read_czti1,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
if(tag ne canale_corrente1) then begin
	...
	nrows1     =fxpar(header,'NAXIS*')
	nrows1     =nrows1(1)
	
	if(wcol gt 0) then BEGIN
	    fxbreadm,unit,[time_unit,picol,wcol,ppcol],tempi1,energy1,weights,peso1,BUFFERSIZE=0   ; energy here is PI
	endif else BEGIN
		fxbreadm,unit,[time_unit,picol],tempi1,energy1,BUFFERSIZE=0
		weights = energy1 * 1.0  ; if no weigths are available, set them to 1
		peso1   = energy1 * 1.0  ; if no peso available, set source to 100%
	endelse
	...
	tempi1   = tempi1[buoni]
	energy1  = energy1[buoni]
	weights = weights[buoni]
	peso1    = peso1[buoni]
	nrows1   = n_elements(buoni)
endif
;
...
;
for irow=irowl,nrows1 do begin
;
...
;
         ...
         if(tbin gt 0.0d0) then begin
           ...
           if (ibin le (np-1)) then begin
			...
			rsour(ibin) = rsour(ibin) + channels(ichan)*peso1(ichan)
