...
;      GH_CZTI
; PURPOSE:
;      Production of CZTI PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from CZTI data
;
; CALLING SEQUENCE:
;       GH_CZTI
;       GH_CZTI,parfilename
;       GH_CZTI,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;					  column exists.
;   QUADRANTS   = selected CZTI quadrants. Array of [1,2,3,4].
;	FRACTION    = minimum pixel exposure fraction for acceptance. Between 0 and 1. 0 means all photons from all pixels.
;
;
; OUTPUTS:
;       NONE
...
;       GH_CZTI,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_CZTI,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;
;   T. Belloni  09 Dec 2015  from GH_LAXPC
...
canale_corrente     = [-1, -1, -1, -1] ; for fxbreadm call
...
canale_corrente0     = -1 ; for fxbreadm call
...
deltagap         = 0.1    ; maximum BTI duration to eliminate
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_czti,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
;-------------------------------------------------------------------
;
;   get desired CZTI quadrants
chout = ''
IF(~keyword_set(quadrants)) THEN BEGIN      ; only if not specified in input
      print,'Choose CZTI quadrants to accumulate'
      print,'ALL: all four quadrants [0,1,2,3]'
      print,'comma separated quadrants (ex: 0,2,3)'
      print,format='($,"(ALL) -> ")'
      read,chout
      IF(chout eq '') THEN BEGIN
         quadrants=[0,1,2,3]
        ENDIF ELSE BEGIN
           quarray       =  strsplit(chout,',',/extract)
           quadrants     = long(quarray)
      ENDELSE
  ENDIF ELSE BEGIN
                  ; future check on keyword input
ENDELSE
nqua = n_elements(quadrants)      ; number of quadrants to use
;-------------------------------------------------------------------
;
;   get desired exposure fraction
chout = ''
IF(~keyword_set(fraction)) THEN BEGIN      ; only if not specified in input
      print,'Choose minimum exposure fraction'
      print,'ALL: all fractions accepted'
      print,'fraction (between 0 and 1)'
      print,format='($,"(0.0) -> ")'
      read,chout
      IF(chout eq '') THEN BEGIN
         fraction=0.0
        ENDIF ELSE BEGIN
		reads,chout,fraction
      ENDELSE
  ENDIF ELSE BEGIN
                  ; future check on keyword input
ENDELSE
...
;
;-------------------------------------------------------------------
;  Here tricky section for CZTI.
;    In order to treat the different quadrants, we are going to mutiply
;      the input filenames. For CZTI there will not be metametafiles, 
;       as they are not needed. So there will be only one @@ (nmetafiles=1) and a list 
;       of nfilenames. We must turn this into four identical lists.
;
        nmetafiles                   = nqua
        filenames_temp               = strarr(nfiles,nmetafiles)
        metafiles_temp               = strarr(nmetafiles)
		metafiles_temp               = metafiles[0];
		FOR iqq = 0,nqua-1 DO BEGIN
			filenames_temp(0:nfiles-1,iqq) = filenames(*,0)
			IF (iqq gt 0) THEN BEGIN
				metafiles_temp                 = [metafiles_temp,metafiles[0]]
			ENDIF
		ENDFOR
					
        ;filenames_temp(0:nfiles-1,0) = filenames(*,0)
        ;filenames_temp(0:nfiles-1,1) = filenames(*,0)
        ;filenames_temp(0:nfiles-1,2) = filenames(*,0)
        ;filenames_temp(0:nfiles-1,3) = filenames(*,0)
        ;metafiles_temp               = [metafiles[0],metafiles[0],metafiles[0],metafiles[0]]
        filenames                    = filenames_temp
        metafiles                    = metafiles_temp
;-------------------------------------------------------------------
;
; obtain start times
;
...
mu_get_time_resolution_czti,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information_czti,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
     nfiles,nmetafiles,nvaltimes, $
         sourcename,obsid,startdate,telescope,instrument,quadrants
...
mu_get_channel_information_czti,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_czti,metafiles,filenames,channels,channels1,channels2, $
...
;
   mu_set_all_channels_czti,metafiles,filenames,channels,channels1,channels2, $
...
rsour = fltarr(np)           ; source light curve (WEIGHTS colunn)
...
endelse
prino,1,'----------------------------------------------------------------',''
prino,1,'Program gh_laxpc    ',systime()+'    User: '+dum
prino,1,'----------------------------------------------------------------',''
...
prino,1,'Quadrants for accumulation      : ',strtrim(string(quadrants),2)
prino,1,'Minimum exposure fraction       : ',strtrim(string(fraction),2)
...
;
; Only for CZTI, merge GTIs when gap is less than deltagap
    ;gh_czti_merge_gtis,valtimes1,valtimes2,nvaltimes,deltagap
;---------------------------------
;
...
;
   for j=0,nmetafiles-1 do begin
      irowl(j)       = 1l
      mu_open_close_czti,unita,filenames(i,j),'open',quadrants(j)
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
	  uu      = files_units[j]
      mu_read_czti,uu,tag,rdata,np,cannali, $
                  fields_selected,j,tress(i,j),tres_fft,   $
                  tstart_fft,tend_fft,$
                  riga,channels_std21,channels_std22,channels_std23,nffts,energy,fraction,quadrants(j),rsour
...
;pippo ;****************************************************************************************************
   ...
   srate = total(rsour)/n_elements(rsour)/tres_fft
   brate = crate - srate
   mu_compute_fft_czti,rdata,pwr,np,T,start_time,source,observatory,           $
          instrument,oufilename,chout,1,0,                   $
          0.0,tstart_fft,new_output_flag,                    $
      output_unit,ntotal_ffts,canali,                        $
	  std21,std22,std23, $
	  BANDE,turbo,gti_flag,proliferation,rsour     ; MU6
   std21 = 0.0
   std22 = 0.0
   std23 = 0.0
;==========================================================================
   ...
   print,'FFT# ',nffts,'/',ntotal_ffts,' Rate/source: ',crate,'/',brate
...
;   close,/all      ; Ultimate choice: why bother?
   for j=0,nmetafiles-1 do begin
      unita=files_units(j)
      mu_open_close_czti,unita,filenames(i,j),'close',j
   endfor
...
close,/all
fine_dps:
print,'gh_czti: normal termination'
;beep
;
