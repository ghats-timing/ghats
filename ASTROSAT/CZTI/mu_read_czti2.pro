pro mu_read_czti2,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
if(tag ne canale_corrente2) then begin
	...
	nrows2     =fxpar(header,'NAXIS*')
	nrows2     =nrows2(1)
	
	if(wcol gt 0) then BEGIN
	    fxbreadm,unit,[time_unit,picol,wcol,ppcol],tempi2,energy2,weights,peso2,BUFFERSIZE=0   ; energy here is PI
	endif else BEGIN
		fxbreadm,unit,[time_unit,picol],tempi2,energy2,BUFFERSIZE=0
		weights = energy2 * 1.0  ; if no weigths are available, set them to 1
		peso2   = energy2 * 1.0  ; if no peso available, set source to 100%
	endelse
	...
	tempi2   = tempi2[buoni]
	energy2  = energy2[buoni]
	weights = weights[buoni]
	peso2    = peso2[buoni]
	nrows2   = n_elements(buoni)
endif
;
...
;
for irow=irowl,nrows2 do begin
;
...
;
         ...
         if(tbin gt 0.0d0) then begin
           ...
           if (ibin le (np-1)) then begin
			...
			rsour(ibin) = rsour(ibin) + channels(ichan)*peso2(ichan)
