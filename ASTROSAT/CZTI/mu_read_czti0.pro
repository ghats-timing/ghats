pro mu_read_czti0,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
if(tag ne canale_corrente0) then begin
	...
	nrows0     =fxpar(header,'NAXIS*')
	nrows0     =nrows0(1)
	
	if(wcol gt 0) then BEGIN
	    fxbreadm,unit,[time_unit,picol,wcol,ppcol],tempi0,energy0,weights,peso0,BUFFERSIZE=0   ; energy here is PI
	endif else BEGIN
		fxbreadm,unit,[time_unit,picol],tempi0,energy0,BUFFERSIZE=0
		weights = energy0 * 1.0  ; if no weigths are available, set them to 1
		peso0   = energy0 * 1.0  ; if no peso available, set source to 100%
	endelse
	canale_corrente0 = tag
	; Here throw away all photons with fractions below threshold (and reset nrows)
	;;;weights = weights*0.0+randomu(12,n_elements(weights))   ; TEMPORARY RANDOMIZATION!!!!!!!!!
	buoni    = where(weights ge fraction)
	tempi0   = tempi0[buoni]
	energy0  = energy0[buoni]
	weights  = weights[buoni]
	peso0    = peso0[buoni]
	nrows0   = n_elements(buoni)
endif
;
...
il_mio = where(time_wanted gt tempi0)+1
irowl  = max(il_mio)+1
...
;
         ...
         if(tbin gt 0.0d0) then begin
           ...
           if (ibin le (np-1)) then begin
			...
			rsour(ibin) = rsour(ibin) + channels(ichan)*peso0(ichan)
