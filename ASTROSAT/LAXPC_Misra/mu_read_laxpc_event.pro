pro mu_read_laxpc_event,unit,laxpc_unit,laxpc_layer,tag,rdata,np,energie,selected,jmeta,tres,tres_fft, $
   ...
   IF((laxpc_unit[quale_unita-1] eq 1) and (laxpc_layer[quale_layer-1] eq 1)) THEN BEGIN 
         ...
         if(tbin gt 0.0d0) then begin
           ...
           if (ibin le (np-1)) then begin
			...
			rdata(ibin) = rdata(ibin) + (energia ge energie[0])*(energia lt energie[1])
			std21       = std21 + (energia ge BANDE[0])*(energia le BANDE[1])
			std22       = std22 + (energia ge BANDE[2])*(energia le BANDE[3])
			std23       = std23 + (energia ge BANDE[4])*(energia le BANDE[5])
