pro mu_get_header_information_xmm,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      if(i eq 0) then begin
		instro[j] = fxpar(header,'INSTRUME')
	  endif
      ...
      extname	  = strtrim(fxpar(header,'EXTNAME'))
	   ...
	   if(instrument eq 'EMOS1')then begin
		types(i,j) = 'mos1'
	   endif
	   if(instrument eq 'EMOS2')then begin
		types(i,j) = 'mos2'
	   endif
	   if((instrument ne 'EPN') and (instrument ne 'EMOS1') and (instrument ne 'EMOS2')) then begin
      ...
      ;timedel= fxpar(header,'TIMEDEL') ; timedel already read in
...
instrument = strtrim(instro[0])
for j=1,nmetafiles-1 do begin
  instrument = instrument+'+'+strtrim(instro[j])
endfor 
