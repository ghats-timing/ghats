...
;      GH_EP
; PURPOSE:
;      Production of Einstein Probe PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from Einstein Probe data
;
; CALLING SEQUENCE:
;       GH_EP
;       GH_EP,parfilename
;       GH_EP,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;					  is a technical PI-channel split, not a calibrated physical energy definition.
...
;       GH_EP,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;    endif else begin
        ...
        massage,'Turbo mode is not enabled for Einstein Probe. Sorry.'
;	endelse
endif else begin
   turbo = 0
endelse
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_ep,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_ep,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_channel_information_ep,filenames,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_ep,metafiles,filenames,channels,channels1,channels2, $
...
endelse
prino,1,'----------------------------------------------------------------',''
prino,1,'Program gh_ep     ',systime()+'    User: '+dum
prino,1,'----------------------------------------------------------------',''
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
      mu_read_ep,unita,tag,rdata,np,cannali, $
...
;
;  time offset is obtained from the FITS MJDREFI/MJDREFF/TIMEZERO keywords:
;  time_offset = dble(mjdrefi) + mjdreff + dble(timezero)/86400.0d0
;  The event TIME values are otherwise used as provided by the EP event file.
;
...
close,/all
fine_dps:
print,'gh_ep: normal termination'
;beep
;
