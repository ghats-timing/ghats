...
;      GH_XTE
; PURPOSE:
;      Production of RXTE PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from RXTE data
;
; CALLING SEQUENCE:
;       GH_XTE
;       GH_XTE,parfilename
;       GH_XTE,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;					  an ASCII file with lines containing start end time of each GTI (in RXTE time units),
...
;       GH_XTE,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;       T. Belloni  31 May 2002  first exportable version (partial)
;       T. Belloni  31 Jul 2002  added opt. for PDS length in s
;       T. Belloni  10 Dec 2003  std2 rate curves added
;       T. Belloni  13 Aug 2008  initial adaptation for GUI input for Mu 4.0
;       T. Belloni  14 Oct 2008  i_vle added 
;       T. Belloni  13 Feb 2009  free_lun changed
;		T. Belloni  20 Feb 2009  MU6 version: naming conventions changed
;		T. Belloni   8 Mar 2009  turbo option added + bands option
;		T. Belloni  17 Mar 2009  full MU6 version	
;		T. Belloni  29 Mar 2009  i_vle propagation
;		T. Belloni  29 Jul 2009  fixed turbo bug across GTI (/NOCROSSGTI now set)
;		T. Belloni   9 Oct 2009  added barycentered option
;		T. Belloni  10 Nov 2009  from MUFFT
;		T. Belloni  16 Nov 2009  new version_id for GHATS
;		T. Belloni  14 Dec 2010  turbo drop outs corrected
;	    T. Belloni   5 Apr 2011  std2_filelist problem fixed (IDL 8?)
;		T. Belloni  22 Nov 2011  checked for turbo mode for FFT
;		T. Belloni   1 Feb 2012  removed prino from RADPS call in turbo
;		T. Belloni   7 Feb 2012  fixed wrong renormalization of NPCUs
;		T. Belloni   7 Feb 2012  fixed turbo not being able to find std2 from outside dir
;		T. Belloni   7 Feb 2012  removed call to fsc_base_filename
;		T. Belloni   8 Feb 2012  input channels restricted to actual values
;		T. Belloni   8 Mar 2012  added windowing
;		T. Belloni  11 May 2012  fixed windowing bug for turbo
...
MAX_N_CHANNELS  =  256
MAX_N_COL       =  256
MAX_VLE         = 6000   ; was 3k in MU5-

;================================================================
if(keyword_set(bands)) then begin
	BANDE           = bands
endif else begin
   ;BANDE           = [0,9,10,31,32,58]  ; 0-13.14-35,36-71 PCA channels
    BANDE           = [3,6,7,14,15,20]
endelse
...
;
i_vle           = 3   ; must be read from calibration/housekeeping data
ndet_def        = 5
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_xte,type,infile,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
if (parameters_known eq 0) then begin
   mu_ask,infilename,'FS37*;FS3b*;FS3f*;FS4f*;@*;e*'
endif
;-------------------------------------------------------------------
;
...
mu_get_time_resolution,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
ioffset   = nchannels
ibit_chan = nchannels
mu_get_channel_information,filenames,channels1,channels2,nchannels,nhistos, $
             ioffset,ibit_chan,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection,metafiles,filenames,channels,channels1,channels2, $
...
if (parameters_known eq 0) then begin
;  interactive
   mu_2string,tress(0,0),chtres
   print,'Available time resolution: ',chtres,' (',tress(0,0),') seconds'
...
;print,'Chosen time resolution : ',chtres,' s (',tres_fft,') s'
;-------------------------------------------------------------------
...
;   snp   = string(double(npds))  ; double for GDL compatibility ??? CRAZY!
   ...
   np=long(nint(16.0/tres_fft))
      print,'Default # of points per FFT ',np,' (16 seconds)'
...
stringa_vle = dirname+'/FH5a*'
; must be uncompressed
vlelist=file_search(stringa_vle,count=quanti)
if(quanti eq 0) then begin
   massage,'No HK file for VLE flag found. Assume 3'
   i_vle = 3
  endif else begin
   vlefile  = vlelist(0)
   gh_getvle,vlefile,i_vle,/nodialog
endelse
;--------------------------------------------------------------------------
; Summary of input
;--------------------------------------------------------------------------
...
prino,1,'Time resolution (s)             : ',strtrim(chtres,2)
...
; Check whether a window file is required
	...
	winn = 0   ; default window is boxcar, for output
...
;
   for j=0,nmetafiles-1 do begin
      ;files_units(j) = 12+j  ; not really needed as fxbopen gives unit
      ;files_tags(j)  = 12+j
      irowl(j)       = 1l
      ;unita=files_units(j)
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      case types(i,j) of
      'pca_sa': begin
;        Single out array of selected channels for current file/metafile
         cannali = channels(*,i,j)
         riga    = 0l
     riga    = irowl(j)   ; subarrays are passed by value!!
         mu_read_pca_sa,unita,rdata,np,cannali, $
                     fields_selected,j,tress(i,j),tres_fft,   $
                     tstart_fft,tend_fft,nchannels(i,j),      $
                     nhistos(i,j),riga,proliferation
      ...
      riga    = 0l
      riga    = irowl(j)  ; subarrays are passed by value!!
; Here MU6 version with bufferized reading
          mu_read_pca_se_buffer,unita,tag,rdata,np,cannali, $
                      fields_selected,j,tress(i,j),tres_fft,   $
                      tstart_fft,tend_fft,nchannels(i,j),      $
                      nhistos(i,j),ioffset(i,j),ibit_chan(i,j),$
                      riga
          irowl(j) = riga  ; subarrays are passed by value!!
       end
      ...
      'pca_gx': begin
          ...
          riga    = 0l
          riga    = irowl(j)  ; subarrays are passed by value!!
          mu_read_pca_gx_buffer,unita,tag,rdata,np,cannali, $
                      fields_selected,j,tress(i,j),tres_fft,   $
                      tstart_fft,tend_fft,nchannels(i,j),      $
                      nhistos(i,j),ioffset(i,j),ibit_chan(i,j),$
                      riga
      ...
      riga    = 0l
      riga    = irowl(j)  ; subarrays are passed by value!!
          mu_read_hexte,unita,rdata,np,cannali, $
                      fields_selected,j,tress(i,j),tres_fft,   $
                      tstart_fft,tend_fft,nchannels(i,j),      $
                      nhistos(i,j),ioffset(i,j),ibit_chan(i,j),$
                      riga
...
;
;  Get VLE count rate at central time of current bin
;
   if(housedata eq 1) then begin
      tmid_fft = tstart_fft + 0.5 * T
      mu_get_vle_rate,tmid_fft,current_vle_rate,ndet,vle_times,    $
                  vle_counts,npcus,nvle
;-----------------------------------------------
;
;   Here Mu3 call to new routine for STD2 fluxes
;
     mu_read_std2_rates,tmid_fft,std21,std22,std23,vle_times,    $
                 rate1,rate2,rate3,nvle
;      read_std2_rates_new,tstart_fft,tend_fft,std21,std22,std23,vle_times,    $
;                 rate1,rate2,rate3,nvle
;-----------------------------------------------
      if((sw_first_segment eq 0) and (ndet_old ne ndet)) then begin
         print,'At time: ',tmid_fft,' NPCU from: ',   $
               ndet_old,' to: ',ndet
      endif
      ndet_old = ndet
    endif else begin
;
;   If no standard2 file was found, then ndet is just set to the default (5)
;   So, if the VLE rates are zero in your FFT file, the ndet is not to be
;   trusted! This is not ideal.
;
      std21            = 0.0
      std22            = 0.0
      std23            = 0.0
...
;
   ...
   mu_compute_fft,rdata,pwr,np,T,start_time,source,observatory,           $
          instrument,oufilename,chout,ndet,i_vle,                   $
          current_vle_rate,tstart_fft,new_output_flag,       $
...
;      if(perc eq 1) then print,' '
;      print,'Percentage done: ',perc  ;TEST
;      perc_old = perc
;   endif
;
...
close,/all
fine_dps:
print,'gh_xte: normal termination'
;beep
;
