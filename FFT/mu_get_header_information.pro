pro mu_get_header_information,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      if(strmid(datamode,0,3) eq 'SBO') then begin
	 massage,'Data type is SBO!'
	 retall
      endif
	 ...
	 types(i,j)='pca_se'
	 if(strmid(datamode,0,9) eq 'GoodXenon') then types(i,j) = 'pca_gx'
      endif
	 ...
	 types(i,j)='hxt_se'
      ...
      if((types(i,j) eq '') or (types(i,j) eq 'hxt_sa')) then begin
	 print,types(i,j)
...
;
		  ...
		  tress(i,j) = timedel/double(nhisto)
		  goto,trecinque
	       endif
	    endif
	 endfor
...
;      status = fxmove(unit,2,/Silent)  ; too slow......_@_Y
	 ...
	 nvaltimes(i,j) = 1
	 valtimes1(1,i,j) = tstarts(i,j)
	 valtimes2(1,i,j) = tends(i,j)
      endif ELSE begin
