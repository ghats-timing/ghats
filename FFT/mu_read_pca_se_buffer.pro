pro mu_read_pca_se_buffer,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
                tstart_fft,tend_fft,nchan,nhisto,                    $
		ioffset,ibit_chan,irowl
;
;  Read in science event files
;
;-------------------------------------------------------------------------
; Parameters
;
...
;         irowl = irowl - 1l
;         irowu = irowl + 1l
;         ;print,'Trouble with event data'  ; removed mu6 2009-Mar-17 to allow for sliding window
;         ;trtc
...
; Type should be BYTE
   ...
   if((idltype(icol) eq 1) and (icol eq 1)) then begin
      ...
      event_byte = [eventi(0,irow-1),eventi(1,irow-1)] ; MU6
;
;     Here the masked filtering must be assigned. It all depends on the
;     little/big endian convention
;
      event=fix(event_byte,0)
      mu_parse_event,event_byte,2,ichan,ioffset,ibit_chan
      ichan=ichan-1   ; start from zero!!
     if(mu_bit_extractor(event,9) eq 0) then begin  ; little endian (x86)
;      if(mu_bit_extractor(event,1) eq 0) then begin  ; big endian (mac)
         time_marker = 0  ; this is a time marker
	endif else begin
	 time_marker = 1
      endelse
     endif else begin
      massage,'Unexpected data format in SE data'
   endelse
;
