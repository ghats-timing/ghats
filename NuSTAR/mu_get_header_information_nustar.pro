pro mu_get_header_information_nustar,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      extname	  = strtrim(fxpar(header,'EXTNAME'))
	   ...
	   if((instrument eq 'FPMA') or (instrument eq 'FPMB')) then begin
		types(i,j) = 'photon'
	   endif else begin
      ...
      ;timedel= fxpar(header,'TIMEDEL')   ; no TIMEDEL in NuSTAR data
      tdim	=fxpar(header,'TDIM*')
      ntdim	=n_elements(tdim)
      tstarts(i,j) = tstart
      tends(i,j)   = tstop
;      tress(i,j)   = timedel
      tress(i,j)   = 0.000002   ; set to 2 ms
;
