pro gh_nustar,infilename,outtype,canali,treb,npds,oufilename,ghx=ghx,turbo=turbo,bands=bands,gti=usergti,sliding=sliding,bary=bary, $
	       wind=wind,wpar=wpar,help=help
;+
; NAME:
;      GH_NUSTAR
; PURPOSE:
;      Production of NuSTAR PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from NuSTAR data
;
; CALLING SEQUENCE:
;       GH_NUSTAR
;       GH_NUSTAR,parfilename
;       GH_NUSTAR,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_NUSTAR,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_NUSTAR,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;       T. Belloni   5 Mar 2013  from GH_SWIFT
;		T. Belloni   4 Dec 2013  fixed time rebinning from I to L
...
if(keyword_set(help)) then begin
   ...
   print,'GH_NUSTAR'
   print,''
   print,'Produce GHATS .pds or .fft files from NuSTAR event data.'
   print,''
   print,'Usage:'
   print,'  GH_NUSTAR'
   print,"  GH_NUSTAR, '#parameters.par'"
   print,"  GH_NUSTAR, infile, outtype, channels, treb, npds, outfile"
   print,''
   print,'Arguments:'
   print,'  infile    NuSTAR event file, @metafile, or @@metametafile'
   print,"  outtype   'POWER' for .pds, or 'FFT' for .fft"
   print,'  channels  [start,end] PHA/PI channel range'
...
;    endif else begin
        ...
        massage,'Turbo mode is not enabled for NuSTAR. Sorry.'
;	endelse
endif else begin
   turbo = 0
endelse
...
if(keyword_set(bands)) then begin
    ...
    BANDE           = [100,800,801,4095,100,4095]
endelse
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_nustar,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_nustar,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information_nustar,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
mu_get_channel_information_nustar,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_nustar,metafiles,filenames,channels,channels1,channels2, $
...
close,/all
fine_dps:
print,'gh_nustar: normal termination'
;beep
;
