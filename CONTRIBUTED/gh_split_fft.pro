PRO gh_split_fft, file_in, gap_threshold=gap_threshold, help=help
;+
; NAME:
;      GH_SPLIT_FFT
;
; PURPOSE:
;      Split a GHATS .fft file into separate files at large time gaps.
;
; CALLING SEQUENCE:
;      GH_SPLIT_FFT, file_in
;      GH_SPLIT_FFT, file_in, GAP_THRESHOLD=gap_threshold
;      GH_SPLIT_FFT, /HELP
;
; INPUTS:
...
;                     when the time gap between consecutive FFT segments is
;                     larger than this value.  Default: 1200 seconds.
;      HELP           Print help and return.
;
; OUTPUTS:
;      Writes files named <input_base>_orbNN.fft.
;
; ROUTINES USED:
;      GHATS_OPENFFT, GHATS_GETHEADER
;
...
;-
  ...
  IF KEYWORD_SET(help) THEN BEGIN
     ...
     PRINT, 'GH_SPLIT_FFT'
     PRINT, ''
     PRINT, 'Split a GHATS .fft file into separate files at large time gaps.'
     PRINT, ''
     PRINT, 'Usage:'
     PRINT, '  gh_split_fft, file_in'
     PRINT, '  gh_split_fft, file_in, gap_threshold=1200.0'
     PRINT, ''
     PRINT, 'Keywords:'
     PRINT, '  GAP_THRESHOLD  Gap threshold in seconds. Default: 1200.'
     PRINT, '  HELP           Print this help and return.'
     PRINT, ''
     PRINT, 'Output files are named <input_base>_orbNN.fft.'
     RETURN
  ENDIF
