...
;      GH_NICER
; PURPOSE:
;      Production of NICER PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from NICER data
;
; CALLING SEQUENCE:
;       GH_NICER
;       GH_NICER,parfilename
;       GH_NICER,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_NICER,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_NICER,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;		T. Belloni  10 Apr 2018  from GH_SWIFT
...
;
; Check whether noisy detector should be excluded
;
nonoise     = 0
IF(~keyword_set(nonoisydet)) THEN BEGIN
	nonoise = 1
ENDIF
;
...
;    endif else begin
        ...
        massage,'Turbo mode is not enabled for NICER. Sorry.'
;	endelse
endif else begin
   turbo = 0
endelse
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_nicer,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_nicer,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
valtimes2 = valtimes1
nvaltimes = lonarr(nfiles,nmetafiles)
mu_get_header_information_nicer,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
mu_get_channel_information_nicer,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_nicer,metafiles,filenames,channels,channels1,channels2, $
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
      mu_read_nicer,unita,tag,rdata,np,cannali, $
                  fields_selected,j,tress(i,j),tres_fft,   $
                  tstart_fft,tend_fft,$
                  riga,channels_std21,channels_std22,channels_std23,nffts,energy,nonoise
...
close,/all
fine_dps:
print,'gh_nicer: normal termination'
;beep
;
