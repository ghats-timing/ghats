pro gh_swift,infilename,outtype,canali,treb,npds,oufilename,ghx=ghx,turbo=turbo,bands=bands,gti=usergti,sliding=sliding,bary=bary, $
	       wind=wind,wpar=wpar,help=help
;+
; NAME:
;      GH_SWIFT
; PURPOSE:
;      Production of Swift PDS and FFT files for the GHATS project
; EXPLANATION:
...
; CALLING SEQUENCE:
;       GH_SWIFT
;       GH_SWIFT,parfilename
;       GH_SWIFT,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_SWIFT,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_SWIFT,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
if(keyword_set(help)) then begin
   ...
   print,'GH_SWIFT'
   print,''
   print,'Produce GHATS .pds or .fft files from Swift/XRT event data.'
   print,''
   print,'Usage:'
   print,'  GH_SWIFT'
   print,"  GH_SWIFT, '#parameters.par'"
   print,"  GH_SWIFT, infile, outtype, channels, treb, npds, outfile"
   print,''
   print,'Arguments:'
   print,'  infile    Swift/XRT event file, @metafile, or @@metametafile'
   ...
   print,''
   print,'Example:'
   print,"  GH_SWIFT, 'event.evt', 'POWER', [0,1023], 1, 4096, 'swift.pds'"
   print,''
   return
endif
;-------------------------------------------------------------
...
;    endif else begin
        ...
        massage,'Turbo mode is not enabled for Swift. Sorry.'
;	endelse
endif else begin
   turbo = 0
endelse
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_swift,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_swift,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information_swift,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
