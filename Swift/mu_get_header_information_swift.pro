pro mu_get_header_information_swift,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      extname	  = strtrim(fxpar(header,'EXTNAME'))
	   ...
	   if((instrument eq 'XRT') and (datamode eq 'PHOTON')) then begin
		types(i,j) = 'photon'
	   endif
	  if((instrument eq 'XRT') and (datamode eq 'WINDOWED')) then begin
		types(i,j) = 'win'
	   endif
	   if(instrument ne 'XRT') then begin
