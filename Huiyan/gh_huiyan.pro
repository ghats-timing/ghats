...
if(keyword_set(help)) then begin
   ...
   print,'GH_HUIYAN'
   print,''
   print,'Produce GHATS .pds or .fft files from Huiyan/HXMT event data.'
   print,''
   print,'Usage:'
   print,'  GH_HUIYAN'
   print,"  GH_HUIYAN, '#parameters.par'"
   print,"  GH_HUIYAN, infile, outtype, channels, grades, treb, npds, outfile"
   print,''
   print,'Arguments:'
   print,'  infile    Huiyan/HXMT event file, @metafile, or @@metametafile'
   print,"  outtype   'POWER' for .pds, or 'FFT' for .fft"
   print,'  channels  [start,end] channel range'
   print,"  grades    event grades to accumulate, e.g. '02'"
...
close,/all
fine_dps:
print,'gh_huiyan: normal termination'
;beep
;
