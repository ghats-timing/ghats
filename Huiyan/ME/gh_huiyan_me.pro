pro gh_huiyan_me,infilename,outtype,canali,treb,npds,oufilename,ghx=ghx,bands=bands,gti=usergti,sliding=sliding,bary=bary, $
	       wind=wind,wpar=wpar,help=help
;+
; NAME:
;      GH_HUIYAN_ME
; PURPOSE:
;      Production of Huiyan/ME PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from Huiyan/ME data
;
; CALLING SEQUENCE:
;       GH_HUIYAN_ME
;       GH_HUIYAN_ME,parfilename
;       GH_HUIYAN_ME,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_HUIYAN_ME,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_HUIYAN_ME,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;		T. Belloni  27 Feb 2019  from GH_HUIYAN_LE
...
if(keyword_set(help)) then begin
   ...
   print,'GH_HUIYAN_ME'
   print,''
   print,'Produce GHATS .pds or .fft files from Huiyan/HXMT ME event data.'
   print,''
   print,'Usage:'
   print,'  GH_HUIYAN_ME'
   print,"  GH_HUIYAN_ME, '#parameters.par'"
   print,"  GH_HUIYAN_ME, infile, outtype, channels, treb, npds, outfile"
   print,''
   print,'Arguments:'
   print,'  infile    Huiyan/HXMT ME event file, @metafile, or @@metametafile'
   ...
   print,''
   print,'Example:'
   print,"  GH_HUIYAN_ME, '@events.lis', 'POWER', [0,1023], 1, 4096, 'hxmt_me.pds'"
   print,''
   return
endif
;-------------------------------------------------------------
...
mu_get_header_information_huiyan_me,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
mu_get_channel_information_huiyan_me,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_huiyan_me,metafiles,filenames,channels,channels1,channels2, $
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
      mu_read_huiyan_me,unita,tag,rdata,np,cannali, $
                  fields_selected,j,tress(i,j),tres_fft,   $
                  tstart_fft,tend_fft,$
                  riga,channels_std21,channels_std22,channels_std23,nffts,sel_grade
