...
;      GH_HUIYAN_HE
; PURPOSE:
;      Production of Huiyan/HE PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from Huiyan/HE data
;
; CALLING SEQUENCE:
;       GH_HUIYAN_HE
;       GH_HUIYAN_HE,parfilename
;       GH_HUIYAN_HE,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_HUIYAN_HE,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_HUIYAN_HE,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
mu_get_header_information_huiyan_he,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
mu_get_channel_information_huiyan_he,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_huiyan_he,metafiles,filenames,channels,channels1,channels2, $
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
      mu_read_huiyan_he,unita,tag,rdata,np,cannali, $
