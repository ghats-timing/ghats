pro mu_get_header_information_huiyan_he,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      extname	  = strtrim(fxpar(header,'EXTNAME'))
	   ...
	   if(instrument eq 'HE') then begin
		types(i,j) = 'Huiyan_ME'
	   endif
	   if(instrument ne 'HE') then begin
