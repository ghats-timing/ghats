pro mu_get_header_information_huiyan_le,filenames,types,tstarts,tends,tress, $
         valtimes1,valtimes2, $
...
; If the date is in old format, cosntruct it in the new one
      ...
      extname	  = strtrim(fxpar(header,'EXTNAME'))
	   ...
	   if(instrument eq 'LE') then begin
		types(i,j) = 'Huiyan_LE'
	   endif
	   if(instrument ne 'LE') then begin
	  ...
	  timedel = 1.0e-3 ; ******** HARDCODED, superseding 1us in the keyword
