pro mu_read_huiyan_le,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
                  tstart_fft,tend_fft,irowl, $
		          channels_std21,channels_std22,channels_std23,nffts,sel_grade
;
...
;gracol      = where(ttype eq 'GRADE   ')+1 ; OLD
gracol      = where(ttype eq 'Event_Type')+1
...
if(tag ne canale_corrente) then begin
	...
	fxbreadm,unit,[time_unit,picol,gracol],tempi,energy,grade
	canale_corrente = tag
endif
;
...
;   in data
;
;         IF(quale_grade eq sel_grade) THEN BEGIN
