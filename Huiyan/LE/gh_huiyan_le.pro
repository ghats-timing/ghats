pro gh_huiyan_le,infilename,outtype,canali,treb,npds,oufilename,ghx=ghx,bands=bands,gti=usergti,sliding=sliding,bary=bary, $
	       wind=wind,wpar=wpar,help=help
;+
; NAME:
;      GH_HUIYAN_LE
; PURPOSE:
;      Production of Huiyan/LE PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from Huiyan/LE data
;
; CALLING SEQUENCE:
;       GH_HUIYAN_LE
;       GH_HUIYAN_LE,parfilename
;       GH_HUIYAN_LE,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_HUIYAN_LE,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_HUIYAN_LE,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;		T. Belloni  15 Mar 2018  from GH_SWIFT
...
if(keyword_set(help)) then begin
   ...
   print,'GH_HUIYAN_LE'
   print,''
   print,'Produce GHATS .pds or .fft files from Huiyan/HXMT LE event data.'
   print,''
   print,'Usage:'
   print,'  GH_HUIYAN_LE'
   print,"  GH_HUIYAN_LE, '#parameters.par'"
   print,"  GH_HUIYAN_LE, infile, outtype, channels, treb, npds, outfile"
   print,''
   print,'Arguments:'
   print,'  infile    Huiyan/HXMT LE event file, @metafile, or @@metametafile'
   ...
   print,''
   print,'Example:'
   print,"  GH_HUIYAN_LE, '@events.lis', 'POWER', [0,1023], 1, 4096, 'hxmt_le.pds'"
   print,''
   return
endif
;-------------------------------------------------------------
