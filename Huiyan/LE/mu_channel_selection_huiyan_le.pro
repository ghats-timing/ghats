pro mu_channel_selection_huiyan_le,metafiles,filenames,channels,channels1,channels2, $
                    nchannels,nfiles,nmetafiles,istart,iend,multiple,resp_flag
;
...
while (andato eq 0) do begin
   ...
   print,'ALL: All channels (0-1500)'
   print,'RANGE: Continuous range of channels'
   ;print,'ENERGY: energy range in keV'
   print,''
   print,format='($,"(ALL) -> ")'
   read,cpar
...
;
		...
		'e': BEGIN
            ...
            while(abs(istart) ge abs(iend)) do begin
               ...
               read,iend
            endwhile
			istart = fix(1536*(istart-0.1)/13.0)
			iend   = fix(1536*(iend  -0.1)/13.0)
			andato=1
		END
   else: begin
