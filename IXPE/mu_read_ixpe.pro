pro mu_read_ixpe,unit,tag,rdata,np,channels,selected,jmeta,tres,tres_fft, $
...
if(tag ne canale_corrente) then begin
	...
	IF(nonoise EQ 0) THEN BEGIN
	  ...
	  ENDIF ELSE BEGIN
	    fxbreadm,unit,[time_unit,picol],tempi,energy
; MM	    fxbreadm,unit,[time_unit,picol,detidcol],tempi,energy,det_id
		; Now move all counts in det_id=14,34 to channel 0
; MM		quelli         = [where(det_id eq 14),where(det_id eq 34)]
; MM		energy[quelli] = 0
; MM		det_id         = 0 ; reset det_id to save space
	ENDELSE
	canale_corrente = tag
endif
;
