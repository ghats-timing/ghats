...
;      GH_IXPE 
; PURPOSE:
;      Production of IXPE  PDS and FFT files for the GHATS project
; EXPLANATION:
;      This procedure, coming from MU, produces
;      PDS and FFT files from IXPE  data
;
; CALLING SEQUENCE:
;       GH_IXPE
;       GH_IXPE,parfilename
;       GH_IXPE,infilename,outtype,channels,treb,npds,outfilename[,/muxana][,/turbo][,bands=B][,gti=GTI][,sliding=SLIDING][,/BARY]
; INPUTS:
...
;       GH_IXPE,'#mysource.par'
;         Takes parameters from the specified parameter file
;       GH_IXPE,'@binned.lis','POWER',[0,35],1,4096,'mysource.pds'
...
;    endif else begin
        ...
        massage,'Turbo mode is not enabled for IXPE. Sorry.'
;	endelse
endif else begin
   turbo = 0
endelse
...
;       npds           = 0l
   ...
   6: begin
        ...
        if((turbo eq 1) and (outtype eq 'FFT')) then begin
     	...
     	print,'Usage: gh_ixpe,infile,outtype,channels,treb,npds,oufile'
     	stop
      end
endcase
;-------------------------------------------------------------------
...
mu_get_time_resolution_ixpe,filenames,nfiles,nmetafiles,tress
;-------------------------------------------------------------------
;
...
mu_get_header_information_ixpe,filenames,types,tstarts,tends,tress,valtimes1,valtimes2, $
...
mu_get_channel_information_ixpe,channels1,channels2,nchannels,nfiles,nmetafiles
;-------------------------------------------------------------------
...
;
   mu_channel_selection_ixpe,metafiles,filenames,channels,channels1,channels2, $
...
;
   ...
   for j=0,nmetafiles-1 do begin
      ...
      riga    = 0l
      mu_read_ixpe,unita,tag,rdata,np,cannali, $
...
close,/all
fine_dps:
print,'gh_ixpe: normal termination'
;beep
;
